...
and creation of the render pipeline was extensive. The actual vulkan functions
do not validate any arguments passed to them, like some other graphics libraries
do. This is done to improve performance but this also makes finding errors in
the program harder as the program might just crash without an error. For this
reason, vulkan has /validation layers/ which can be enabled in a debug build
that check every argument passed to vulkan functions according to the
specification. So our first steps in vulkan were to set up a debug build which
enables the validation layers.

Next to use the vulkan functions we decided to use the vulkan loader to fetch us
the correct function pointers from the graphics driver on startup, for the
functions that we need instead of linking against vulkan, as the linked vulkan
would itself lookup the function pointers at runtime, so we can save some
performance there.

On application start our program first creates a /vulkan instance/ from which we
can query the hardware and physical devices. When we decided on a physical
device we want to use for rendering we create a "logical device" from it that is
used on a higher level to access the gpu. Each physical device
...
an interpolation type. We can animate any variable in memory. For example we
animate the camera's field of view when finishing a level. But it would also be
possible to animate single vertices or material parameters or anything else
(which however we do not do at the current moment).
