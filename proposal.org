...
game we first consider the cube as a base shape, as it allows for the simpler
square tiling. At the same time we keep the possibility of the tetrahedron as
the second possible player controlled object.

We wanted to add this one more mechanic to our game -- an ability that the
player has, that will let us design deeper and more interesting puzzles. A
sample unfolding of the cube can be seen in Figure [[fig:unfolding_blender]].
